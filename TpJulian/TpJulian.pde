...
void mouseMoved() {
  if (estado == 0) {
    ...
    if (mouseX >= 640 && mouseX <= 640 + 230 && mouseY >= 450 && mouseY <= 450 + 100) {
      estado = 1;
    }
  }
  else if (estado == 1) {
    ...
    float d = dist(mouseX, mouseY, ellipseX, ellipseY); // Calcula la distancia entre el mouse y el centro del óvalo
    ...
    if (mouseX >= 595 && mouseX <= 595 + 235 && mouseY >= 575 && mouseY <= 575 + 100) {
      resetJuego();
    }
  }
  else if (estado == 3) {
    // Verifica si el mouse está sobre el rectángulo en el estado 2
    if (mouseX >= 610 && mouseX <= 610 + 235 && mouseY >= 575 && mouseY <= 575 + 100) {
      resetJuego();
    }
  }
}
