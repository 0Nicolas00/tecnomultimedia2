...
void draw() {
  ...
  if (estado==1) {
    ...
    rect(0, 0, 1000, 700);
    fill(0);
    textSize(12); // Cambia el tamaño según tus necesidades
    // text("Disparar", 150, 150);
    c.actualizarAngulo();
    mundo.step();
    mundo.draw();
    //m.dibujar();
...
void mouseMoved() {
  if (estado == 1) {
    ...
    if (mouseX >= rectX && mouseX <= rectX + rectWidth && mouseY >= rectY && mouseY <= rectY + rectHeight && !disparoRealizado) {
      c.disparar(mundo);
      lanzamiento.play();
